...
multiple changes before a render deliver the latest title. Ghostty ignores
titles longer than 255 bytes.
...
With SGR mouse reporting, mode 1003 sends unpressed pointer motion once per
cell through `onData`. Without SGR, the DOM layer sends X10 mouse reports
through `onBinary` when provided. `mouseEncoding()` exposes Ghostty's active
format. The DOM layer handles X10 and SGR; it does not send 1005, 1015, or
1016 reports.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><code>imageStorageLimit</code></td>
      <td><code>number</code></td>
      <td>Maximum decoded Kitty image bytes per screen (default: 32 MiB). Set to <code>0</code> to disable graphics. One image is capped at <code>MAX_IMAGE_BYTES</code> (32 MiB).</td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>~26 KB WASM (inlined)</td>
      <td>~580 KB WASM (fetched)</td>
    </tr>
    <tr>
      <td>VT compliance</td>
