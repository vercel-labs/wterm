...
With mouse tracking enabled, mode 1003 sends unpressed pointer motion once per
cell. `mouseEncoding()` exposes Ghostty's active wire format. The DOM layer
sends UTF-8 (1005), SGR (1006), and urxvt (1015) reports through `onData`;
X10 reports use `onBinary` when provided. Pixel mouse reports (1016) are not
sent.
