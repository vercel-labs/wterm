...
The live terminal exposes input semantics and mounted output. Use `readText()` to present retained history in an accessible reader; live output announcements are not enabled. Ancestor `aria-hidden` and `inert` remain authoritative for hidden terminals.
