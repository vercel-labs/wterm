...
This exposes input semantics and mounted output; it does not enable output announcements or make unmounted history available to screen readers. Ancestor `aria-hidden` and `inert` remain authoritative for hidden terminals.
