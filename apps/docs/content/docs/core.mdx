...
With `@wterm/dom`, terminal applications can enable mouse tracking (modes 1000, 1002, and 1003) in SGR (1006) or X10 encoding, plus focus reporting (mode 1004). Mode 1003 reports pointer motion without a pressed button once per cell in the visible grid. SGR and focus reports reach `onData`; raw X10 reports reach `onBinary` when provided.
