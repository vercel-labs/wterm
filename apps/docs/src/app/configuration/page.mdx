...
The React, Svelte, and Vue `Terminal` components and the vanilla `WTerm` constructor accept the same core options — `cols`, `rows`, `core`, `wasmUrl`, `autoResize`, `maxImageWidth`, `maxImageHeight`, `cursorBlink`, and `debug` — plus event callbacks `onData`, `onTitle`, and `onResize` (exposed as callback props in Svelte and as `@data`, `@title`, `@resize` events in Vue).
...
The Svelte `Terminal` component adds a `theme` prop. `onReady` and `onError` are callback props. Standard DOM attributes (`class`, `style`, `id`, ARIA props, etc.) are forwarded to the root `<div>`.

See [Svelte-Only Props](/api-reference#svelte-only-props) and [Svelte Callback Props](/api-reference#svelte-callback-props) for details.
...
Access imperative methods on the Svelte component via `bind:this`:
...
See the full [Component Ref (Svelte)](/api-reference#component-ref-svelte) reference for all exposed methods.
