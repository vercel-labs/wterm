...
The vanilla `WTerm` constructor, the React/Vue terminal components, and the Svelte `wterm` action all accept these options:
...
The Vue `<Terminal>` component adds these props on top of the shared options above. It also forwards standard attributes (`class`, `style`, `id`, `data-*`, etc.) onto the root `div`.
...
The `wterm` action adds these options on top of the shared options above.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>TerminalController</code></td>
      <td>Optional imperative controller for writing to, resizing, or focusing the terminal from outside the action</td>
    </tr>
    <tr>
      <td><code>onReady</code></td>
      <td><code>(wt: WTerm) =&gt; void</code></td>
...
The Vue `useTerminal` composable returns a function ref plus convenience methods:

```ts
const { ref, attach, write, resize, focus } = useTerminal();
```
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>ref</code></td>
      <td><code>ShallowRef&lt;TerminalHandle | null&gt;</code></td>
      <td>Current imperative terminal handle</td>
    </tr>
    <tr>
      <td><code>attach</code></td>
      <td><code>(instance: TerminalHandle | null) =&gt; void</code></td>
      <td>Function ref to pass to <code>&lt;Terminal :ref=&#123;attach&#125; /&gt;</code></td>
    </tr>
    <tr>
      <td><code>write</code></td>
      <td><code>(data: string | Uint8Array) =&gt; void</code></td>
...
The Vue component exposes the same imperative handle shape as React:
...
## Terminal Controller (Svelte)

Create a controller when you want imperative access outside the action:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Read-only access to the underlying terminal instance</td>
    </tr>
  </tbody>
</table>
