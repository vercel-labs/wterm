...
Use `<Terminal aria-label="Build shell" aria-describedby="shell-help" />` to name and describe the actual input. `aria-labelledby` and `aria-description` are also supported, and prop changes stay synchronized. The host defaults to a group; the native textarea is the editable control. `tabIndex={-1}` excludes input from page tab entry; the imperative `focus()` method still focuses it. See the [DOM input accessibility contract](../dom/README.md#input-accessibility) for host tab-order ownership and output limits.
