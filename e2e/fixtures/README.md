...
`protocol.ts` contains small authored streams for byte-fragmented Unicode and SGR, DEC line drawing and character-set shifts, wide-character insertion/deletion and right-edge wrapping, alternate-screen restoration, cursor responses, repeated resizing, retained history, and synchronized output. These streams have no capture clock; their `atMs` values are zero. They are not application recordings.
